    ...
    def visiblePoints(self, points: List[List[int]], angle: int, location: List[int]):
        ...
        for x, y in points:
            ...
            if not (x or y):
                onPoint += 1
                continue
            theta = ((atan2(abs(y), abs(x))) * 180) / pi
            if x < 0 and y < 0:
                theta += 180
            elif x < 0:
                theta = 180 - theta
            ...
            angles.append(theta)
            angles.append(360 + theta)

        angles.sort()
        i = j = answer = 0
        while i < len(angles) and j < len(angles):
